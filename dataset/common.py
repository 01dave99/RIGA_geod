...
def collate_fn(list_data, config):
    ...
    batched_src_lenghts_list = []
    batched_tgt_points_list = []
    batched_tgt_lenghts_list = []

    batched_src_node_xyz = []
    batched_tgt_node_xyz = []

    batched_src_node_geod = []
    batched_tgt_node_geod = []

    batched_src_patch_xyz = []
    batched_tgt_patch_xyz = []

    batched_src_patch_feat = []
    batched_tgt_patch_feat = []

    batched_src_p2n_inds = []
    batched_tgt_p2n_inds = []

    batched_src_p2n_masks = []
    batched_tgt_p2n_masks = []

    batched_gt_patch_corr = []
    ...
    batched_src_knn_node_inds = []
    batched_tgt_knn_node_inds = []
    cur_len_src = 0
    cur_len_tgt = 0

    for ind, (src_pcd, tgt_pcd, src_node_xyz, tgt_node_xyz, src_node_geod, tgt_node_geod, src_patch_xyz, tgt_patch_xyz, src_patch, tgt_patch,
              src_p2n_inds, tgt_p2n_inds, src_p2n_masks, tgt_p2n_masks, gt_patch_corr,
              rot, trans, pos_mask, src_knn_node_inds, tgt_knn_node_inds) in enumerate(list_data):
        batched_src_points_list.append(src_pcd)
        batched_tgt_points_list.append(tgt_pcd)
        batched_src_lenghts_list.append(src_pcd.shape[0])
        batched_tgt_lenghts_list.append(tgt_pcd.shape[0])

        batched_src_node_xyz.append(src_node_xyz[np.newaxis, :])
        batched_tgt_node_xyz.append(tgt_node_xyz[np.newaxis, :])

        batched_src_node_geod.append(src_node_geod[np.newaxis, :])
        batched_tgt_node_geod.append(tgt_node_geod[np.newaxis, :])

        batched_src_patch_xyz.append(src_patch_xyz[np.newaxis, :])
        batched_tgt_patch_xyz.append(tgt_patch_xyz[np.newaxis, :])

        batched_src_patch_feat.append(src_patch[np.newaxis, :])
        batched_tgt_patch_feat.append(tgt_patch[np.newaxis, :])

        batched_gt_patch_corr.append(gt_patch_corr[np.newaxis, :])

        batched_rot.append(rot[np.newaxis, :])
        batched_trans.append(trans[np.newaxis, :])

        batched_pos_mask.append(pos_mask[np.newaxis, :])

        src_p2n_inds = src_p2n_inds + cur_len_src
        tgt_p2n_inds = tgt_p2n_inds + cur_len_tgt

        batched_src_p2n_inds.append(src_p2n_inds[np.newaxis, :])
        batched_tgt_p2n_inds.append(tgt_p2n_inds[np.newaxis, :])

        batched_src_p2n_masks.append(src_p2n_masks[np.newaxis, :])
        batched_tgt_p2n_masks.append(tgt_p2n_masks[np.newaxis, :])

        src_knn_node_inds += src_node_xyz.shape[0] * ind
        tgt_knn_node_inds += tgt_node_xyz.shape[0] * ind

        batched_src_knn_node_inds.append(src_knn_node_inds)
        batched_tgt_knn_node_inds.append(tgt_knn_node_inds)

        cur_len_src += src_pcd.shape[0]
        cur_len_tgt += tgt_pcd.shape[0]
    ...
    batched_src_lengths = torch.from_numpy(np.array(batched_src_lenghts_list)).int()
    batched_tgt_lengths = torch.from_numpy(np.array(batched_tgt_lenghts_list)).int()

    batched_src_node_xyz = torch.from_numpy(np.concatenate(batched_src_node_xyz, axis=0))
    batched_tgt_node_xyz = torch.from_numpy(np.concatenate(batched_tgt_node_xyz, axis=0))

    batched_src_node_geod = torch.from_numpy(np.concatenate(batched_src_node_geod, axis=0))
    batched_tgt_node_geod = torch.from_numpy(np.concatenate(batched_tgt_node_geod, axis=0))

    batched_src_patch_xyz = torch.from_numpy(np.concatenate(batched_src_patch_xyz, axis=0))
    batched_tgt_patch_xyz = torch.from_numpy(np.concatenate(batched_tgt_patch_xyz, axis=0))

    batched_src_patch_feat = torch.from_numpy(np.concatenate(batched_src_patch_feat, axis=0))
    batched_tgt_patch_feat = torch.from_numpy(np.concatenate(batched_tgt_patch_feat, axis=0))

    batched_src_p2n_inds = torch.from_numpy(np.concatenate(batched_src_p2n_inds, axis=0))
    batched_tgt_p2n_inds = torch.from_numpy(np.concatenate(batched_tgt_p2n_inds, axis=0))

    batched_src_p2n_masks = torch.from_numpy(np.concatenate(batched_src_p2n_masks, axis=0))
    batched_tgt_p2n_masks = torch.from_numpy(np.concatenate(batched_tgt_p2n_masks, axis=0))

    batched_gt_patch_corr = torch.from_numpy(np.concatenate(batched_gt_patch_corr, axis=0))
    ...
    batched_pos_mask = torch.from_numpy(np.concatenate(batched_pos_mask, axis=0))

    batched_src_knn_node_inds = torch.from_numpy(np.concatenate(batched_src_knn_node_inds, axis=0))
    batched_tgt_knn_node_inds = torch.from_numpy(np.concatenate(batched_tgt_knn_node_inds, axis=0))
    ...
    inputs = {
        ...
        'src_lengths': batched_src_lengths,
        'tgt_points': batched_tgt_points,
        'tgt_lengths': batched_tgt_lengths,
        'src_nodes': batched_src_node_xyz,
        'tgt_nodes': batched_tgt_node_xyz,
        'src_node_geod': batched_src_node_geod,
        'tgt_node_geod': batched_tgt_node_geod,
        'src_patch_xyz': batched_src_patch_xyz,
        'tgt_patch_xyz': batched_tgt_patch_xyz,
        'src_patch_feat': batched_src_patch_feat,
        'tgt_patch_feat': batched_tgt_patch_feat,
        'src_p2n_inds': batched_src_p2n_inds,
        'tgt_p2n_inds': batched_tgt_p2n_inds,
        'src_p2n_masks': batched_src_p2n_masks,
        'tgt_p2n_masks': batched_tgt_p2n_masks,
        'gt_patch_corr': batched_gt_patch_corr,
        'rot': batched_rot,
        'trans': batched_trans,
        'pos_mask': batched_pos_mask,
        'src_knn_node_inds': batched_src_knn_node_inds,
        'tgt_knn_node_inds': batched_tgt_knn_node_inds
    }
    return inputs
...
def calc_geod(point1, normal1, point2, normal2, geodesic, neighbors):
    '''Given a representative point and a patch of points, calculate geodesic distamces for each of the point in this patch
    :param point1: xyz coordinates of the representative point of numpy.ndarray in shape[n, 3]
    :param normal1: normal of the representative point of numpy.ndarray in shape[n, 3]
    :param point2: xyz coordinates of a patch of points of numpy.ndarray in shape[m, 3]
    :param normal2: normal of representative point of numpy.ndarray in shape[m, 3]
    :param neighbors: patches of point indices of numpy.ndarray in shape[n, num_point_per_patch]
    :param geodesic: geodesic distance from all sampeled points to all points [n,m]
    :return: point pair geodesic ditsmaces of each point in this patch, of numpy.ndarray in shape[n, num_point_per_patch, 1]
    '''
    geod = geodesic[0,neighbors[0]]
    for i in range(1,point1.shape[0]):
        geod=np.concatenate((geod,geodesic[i,neighbors[i]]),axis=0)
    geod=np.reshape(geod,[point1.shape[0],-1]) #[n,num_points_per_patch]
